...
def load_data() -> pd.DataFrame:
    ...
    logger.info("Data loaded in %.3fs: shape=%s, columns=%s", elapsed, df.shape, list(df.columns))
...
def get_feature_types(df: pd.DataFrame) -> Tuple[List[str], List[str], List[str]]:
    ...
    logger.debug("Feature types: numeric=%s, categorical=%s, binary=%s",
...
def build_preprocessor(numeric_features: List[str],
                       ...
                       binary_features: List[str]) -> ColumnTransformer:
...
def evaluate_regression(model, X, y, cv, model_name: str) -> Dict[str, float]:
    ...
    logger.info("  R²:     %.4f (±%.4f) [train: %.4f]",
                metrics['r2_mean'], metrics['r2_std'], metrics['r2_train_mean'])
    ...
    if overfit_gap > 0.1:
        logger.warning("  [WARN] Possible overfitting: train R² - test R² = %.4f", overfit_gap)
...
def evaluate_classification(model, X, y, cv, model_name: str) -> Dict[str, float]:
    ...
    logger.info("  CV completed in %.2fs", elapsed)
    logger.info("  Accuracy: %.4f (±%.4f) [train: %.4f]",
                metrics['accuracy_mean'], metrics['accuracy_std'], metrics['accuracy_train_mean'])
    logger.info("  F1 (w):   %.4f (±%.4f)", metrics['f1_weighted_mean'], metrics['f1_weighted_std'])
    logger.info("  F1 (macro): %.4f (±%.4f)", metrics['f1_macro_mean'], metrics['f1_macro_std'])
    ...
    if overfit_gap > 0.1:
        logger.warning("  [WARN] Possible overfitting: train Acc - test Acc = %.4f", overfit_gap)
...
def tune_regression_model(base_model, param_grid: Dict, X, y, cv, model_name: str):
    ...
    logger.info("Tuning hyperparameters for %s...", model_name)
    start = time.time()

    grid = GridSearchCV(
        base_model, param_grid, cv=cv, scoring='r2', n_jobs=-1,
...
def train_regression_models(X, y, cv) -> Tuple[Tuple[str, Any], Dict]:
    """Train and compare regression models with optional tuning."""
    log_section("REGRESSION: Predicting Final_Score")

    # Define models with tuning grids
    model_configs = {
        'RandomForest': {
    ...
    for name, config in model_configs.items():
        ...
        if config['tune'] and config['param_grid']:
            ...
            tuning_info[name] = {'best_params': best_params, 'best_cv_score': best_cv_score}
        else:
            # No tuning needed (e.g., RidgeCV)
            model = config['model']
            metrics = evaluate_regression(model, X, y, cv, name)
            tuned_model = model
            tuning_info[name] = {'best_params': 'N/A (built-in CV)', 'best_cv_score': metrics['r2_mean']}
    ...
    logger.info("\nBest regression model: %s (R² = %.4f)", best_model[0], best_score)
    return best_model, results, tuning_info


def train_classification_models(X, y, cv) -> Tuple[Tuple[str, Any], Dict]:
    """Train and compare classification models with optional tuning."""
    ...
    model_configs = {
        ...
        'LogisticRegressionCV': {
            'model': LogisticRegressionCV(Cs=[0.01, 0.1, 1.0, 10.0, 100.0],
    ...
    for name, config in model_configs.items():
        ...
        if config['tune'] and config['param_grid']:
            ...
            tuned_model = model
            tuning_info[name] = {'best_params': 'N/A (built-in CV)', 'best_cv_score': metrics['f1_weighted_mean']}
    ...
    logger.info("\nBest classification model: %s (Accuracy = %.4f)", best_model[0], best_score)
    return best_model, results, tuning_info
...
# ============================================================
def compute_permutation_importance(model, X, y, feature_names: List[str],
                                    model_name: str, n_repeats: int = 10) -> Dict:
    ...
    for i in sorted_idx[:10]:
        logger.info("    %s: %.4f (±%.4f)", feature_names[i],
                    result.importances_mean[i], result.importances_std[i])
...
# PLOTTING (ENHANCED)
# ============================================================
def get_feature_names_after_preprocessing(preprocessor, original_names: Tuple) -> List[str]:
    """Get feature names after one-hot encoding."""
    try:
        cat_encoder = preprocessor.named_transformers_['cat']
        cat_features = cat_encoder.get_feature_names_out()
        all_features = np.concatenate([
            original_names[0],  # numeric
            cat_features,
            original_names[2]   # binary
        ])
...
def plot_feature_importance(model, preprocessor, feature_names: Tuple,
...
def main():
    ...
    logger.info("Target regression range: [%d, %d], mean=%.2f",
                y_reg.min(), y_reg.max(), y_reg.mean())
    logger.info("Target classification distribution: %s",
                dict(zip(*np.unique(y_clf, return_counts=True))))

    # Fit preprocessor
    X_transformed = preprocessor.fit_transform(X)
    logger.info("Transformed feature shape: %s", X_transformed.shape)

    # Get final feature names for interpretation
    final_feature_names = get_feature_names_after_preprocessing(preprocessor, feature_names)
    logger.debug("Final feature names (%d): %s", len(final_feature_names), final_feature_names)
    ...
    clf_start = time.time()
    (best_clf_name, best_clf_model), clf_results, clf_tuning = train_classification_models(
    ...
    metrics = {
        ...
        'data_info': {
            ...
            'target_clf_distribution': {str(k): int(v) for k, v in zip(*np.unique(y_clf, return_counts=True))},
    ...
    plot_feature_importance(
        ...
        OUTPUT_DIR / "feature_importance_regression.png", best_reg_name
    )
    plot_feature_importance(
        best_clf_model, preprocessor, feature_names,
        OUTPUT_DIR / "feature_importance_classification.png", best_clf_name
    )
    plot_predictions_vs_actual(
        best_reg_model, X_transformed, y_reg,
        OUTPUT_DIR / "predictions_vs_actual_regression.png", "Final Score"
    )
    plot_confusion_matrix(
        best_clf_model, X_transformed, y_clf,
        OUTPUT_DIR / "confusion_matrix.png", best_clf_name
    )
    plot_learning_curves(
        best_reg_model, X_transformed, y_reg,
        OUTPUT_DIR / "learning_curve_regression.png", best_reg_name
    )
    plot_learning_curves(
        best_clf_model, X_transformed, y_clf,
        OUTPUT_DIR / "learning_curve_classification.png", best_clf_name
    )
    ...
    logger.info("Best regression: %s (R² = %.4f ± %.4f)",
                best_reg_name,
                reg_results[best_reg_name]['r2_mean'],
                reg_results[best_reg_name]['r2_std'])
    logger.info("Best classification: %s (Acc = %.4f ± %.4f, F1 = %.4f)",
                best_clf_name,
