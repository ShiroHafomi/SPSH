 ...
 * Icons now use Lucide icon names (rendered as SVG on frontend).
 */
...
const RECOMMENDATIONS = {
  ...
  gradeAdvice: (grade, score) => ({
    A: { icon: 'Star', severity: 'success', title: 'Outstanding Performance', text: `Predicted grade: A (${score}/100). Your habits are excellent — keep challenging yourself. Consider mentoring peers or seeking advanced supplementary materials.` },
    B: { icon: 'TrendingUp', severity: 'success', title: 'Good Performance', text: `Predicted grade: B (${score}/100). You're doing well! Identify which skills cost you points — is it exams, assignments, or participation? Target that one area for improvement.` },
    C: { icon: 'Edit', severity: 'warning', title: 'Room for Improvement', text: `Predicted grade: C (${score}/100). There's potential! The biggest lever is study hours: 30 more minutes per day can push you from C to B territory. Create a narrow plan.` },
    D: { icon: 'AlertTriangle', severity: 'danger', title: 'Needs Attention', text: `Predicted grade: D (${score}/100). Immediate intervention recommended: attend office hours, form a group, review fundament tasks, practice with past exam problems daily.` },
    F: { icon: 'AlertTriangle', severity: 'danger', title: 'At Risk of Failing', text: `Predicted grade: F (${score}/100). This is serious. Speak with your academic advisor immediately. Request a study skills workshop, peer tutor or academic probation support. There ARE resources available.` },
  }[grade] || {}),
};
...
function generateFeedback(profile, prediction) {
  ...
  // Build share text (using icon names for text representation)
