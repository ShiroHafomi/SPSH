            ...
            <ResponsiveContainer width="100%" height="100%">
              <ScatterChart>
                ...
                <Tooltip
                  labelFormatter={(_, payload) => {
                    ...
                    if (item && item.value && typeof item.value === 'object') {
                      const value = item.value as { x?: number; y?: number };
                      return `Study Hours: ${value.x?.toFixed(1)}, Score: ${value.y?.toFixed(1)}`;
                    }
                    return '';
                  }}
                ...
                <Scatter
                  name="Trend"
                  data={(function() {
                    const scatterData = chart.data as { x: number; y: number }[];
                    const sorted = scatterData.slice().sort((a, b) => a.x - b.x);
                    const avg = sorted.reduce((s, p) => s + p.y, 0) / sorted.length;
                    return sorted.map((d) => ({ x: d.x, y: avg }));
                  })()}
                  fill="transparent"
                  stroke="#22c55e"
                  strokeDasharray="5 5"
    ...
    return (
      <Card className="mt-6">
        ...
        <CardContent>
          <div className="h-[300px] w-full">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={chart.labels.map((label, i) => ({ label, value: chartData[i] }))}>
                <CartesianGrid strokeDasharray="3 3" />
    ...
    return (
      <Card className="mt-6">
        ...
        <CardContent>
          <div className="h-[300px] w-full">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={chart.labels.map((label, i) => ({ category: label, value: chartData[i] }))} layout="vertical">
                <CartesianGrid strokeDasharray="3 3" />
