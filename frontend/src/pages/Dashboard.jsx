...
export default function Dashboard() {
  ...
  // KPI cards: total students + at-risk are featured/actionable; the rest are metric cards.
  const icons = [
  ...
  const cards = useMemo(() => {
    const atRiskCount =
      atRisk && typeof atRisk.count === 'number' ? atRisk.count : 0;
    const list = [];
    kpis.forEach((kpi) => {
  ...
  return (
    <div className="space-y-8">
      {/* KPI Cards — balanced 3+2 bento with featured, actionable cards */}
      <div className="grid gap-5 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3">
        {cards.map((card, idx) => {
          if (card.featured) {
            return (
              ...
              >
                <div className="flex items-start justify-between">
                  <p className="text-xs font-bold uppercase tracking-widest text-white/75">{card.label}</p>
                  <div className="w-11 h-11 rounded-2xl bg-white/15 flex items-center justify-center text-white flex-shrink-0">
                    {card.danger ? (<svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-2.5L13.732 4c-.77-.833-1.964-.833-2.732 0L4.082 16.5c-.77.833.192 2.5 1.732 2.5z" /></svg>) : (<svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 7h8m0 0v8m0-8l-8 8-4-4-6 6" /></svg>)}
                  </div>
                </div>
                <p className="text-4xl font-extrabold text-white tabular-nums font-mono">{formatKPI(card.value, card.format)}</p>
          ...
          return (
            <div key={card.key || idx} className="kpi-card group">
              <div className="flex items-start justify-between">
                <p className="kpi-label">{card.label}</p>
                <div className={`kpi-icon-box bg-gradient-to-br ${card.gradient || gradients[idx % 4]} text-white shadow-clay-sm`}>
                  {icons[idx % 4]}
                </div>
              </div>
              <p className="kpi-value">{formatKPI(card.value, card.format)}</p>
            </div>
          );
        })}
      </div>
      ...
      {atRisk && atRisk.count > 0 && (
        <div className="card border-l-4 border-danger-500 bg-danger-50/60 dark:bg-danger-950/20 p-5">
          <div className="flex items-start justify-between gap-4">
            ...
            <Link to="/students?at_risk=1" className="btn-danger flex-shrink-0">{t('dashboard.viewStudents')}</Link>
          </div>
        </div>
      )}
