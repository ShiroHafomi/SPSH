...
export default function AdminAtRisk() {
  ...
  const fetchStudents = useCallback(async () => {
    try {
      ...
      // Filter for at-risk students only (those with risk_level not low or null)
      const atRiskStudents = (data.students || []).filter(s => s.risk_level && s.risk_level !== 'low');
      setStudents(atRiskStudents);
      setTotal(atRiskStudents.length);
    } catch (err) {
  ...
  const handleGenerateIntervention = async (student) => {
    ...
    try {
      ...
      setInterventionNote(data.intervention_note || 'Generated intervention note');
  ...
  const handleSaveIntervention = async () => {
    ...
    try {
      // The intervention is already saved by the backend, just close modal
      addFlash({ type: 'success', message: 'Intervention note saved to student record' });
  ...
  const handleExport = async () => {
    try {
      ...
      addFlash({ type: 'success', message: 'At-risk students exported successfully' });
    } catch (err) {
      addFlash({ type: 'error', message: 'Failed to export at-risk students' });
    } finally {
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
        <div>
          ...
          <p className="text-primary-500 dark:text-gray-400 mt-1">Students flagged by attendance, sleep, GPA, and study habit thresholds</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="rounded-2xl p-4 border bg-error-50 dark:bg-error-900/20 border-error-200 dark:border-error-800">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm font-medium text-error-700 dark:text-error-300">High Risk</p>
      ...
      {showFilters && (
        <div className="rounded-2xl p-4 border bg-white/80 dark:bg-gray-800/80 backdrop-blur-xl shadow-clay-sm animate-slide-down">
          <div className="flex flex-col sm:flex-row gap-4 mb-4">
            ...
            <SelectFilter label="Risk Level" value={filters.risk_level} options={RISK_LEVELS} onChange={(v) => handleFilterChange('risk_level', v)} />
          </div>
          {hasActiveFilters && (
                ...
                <tr>
                  <td colSpan={11} className="px-4 py-12 text-center">
                    <div className="flex flex-col items-center gap-3 text-primary-500 dark:text-gray-400">
                      ...
                      <p className="font-medium">No at-risk students found</p>
                      <p className="text-sm">All students are currently on track based on the risk thresholds</p>
                    </div>
                  </td>
                </tr>
              ) : (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-fade-in">
          <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-xl max-w-2xl w-full max-h-[80vh] overflow-hidden animate-slide-up">
            <div className="flex items-center justify-between p-4 border-b border-primary-100 dark:border-gray-800">
              <h2 className="text-lg font-semibold text-primary-950 dark:text-gray-100">AI Academic Intervention</h2>
            ...
            <div className="p-4 max-h-[60vh] overflow-y-auto">
              <div className="mb-4 p-3 bg-primary-50 dark:bg-primary-900/20 rounded-xl">
                <p className="text-sm font-medium text-primary-950 dark:text-gray-100">Student: {selectedStudent.name} ({selectedStudent.student_id})</p>
                <p className="text-sm text-primary-600 dark:text-primary-400 mt-1">Risk Level: <RiskBadge riskLevel={selectedStudent.risk_level} /></p>
              </div>
              <div className="space-y-3">
                <label className="block text-sm font-medium text-primary-700 dark:text-gray-300">Intervention Note</label>
                <textarea
                  ...
                  placeholder="AI-generated intervention note will appear here..."
                />
              </div>
            </div>
